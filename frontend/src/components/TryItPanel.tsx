          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-sm text-foreground">{endpoint.title}</span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin p-6 space-y-6">
              {/* URL */}
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block mb-2">Endpoint</label>
              ...
              {endpoint.params && endpoint.params.length > 0 && (
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block mb-2">Parameters</label>
                  <div className="space-y-2">
                    {endpoint.params.map((p) => (
                      <div key={p.name}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-mono text-primary">{p.name}</span>
                          {p.required && <span className="text-[9px] font-semibold px-1 py-0.5 rounded bg-method-delete/15 text-method-delete">REQ</span>}
                        </div>
                        <input
              ...
              {endpoint.requestBody && (
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block mb-2">Request Body</label>
                  <textarea
                    value={body}
              ...
              <AnimatePresence>
                {response && (
                  ...
                  >
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block mb-2">Response</label>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="h-2 w-2 rounded-full bg-method-get animate-pulse" />
                      <span className="text-xs font-mono text-method-get">200 OK</span>
                      <span className="text-xs text-muted-foreground">• 127ms</span>
                    </div>
